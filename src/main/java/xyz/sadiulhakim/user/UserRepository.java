...
interface UserRepository extends JpaRepository<User, UUID> {
    ...
    @Query(value = "select new xyz.sadiulhakim.user.pojo.UserDTO(u.id,u.firstname,u.lastname,u.email,u.picture,u.textColor,u.lastSeen) " +
            "from User u where u.email= :email")
    UserDTO findByEmailProjection(@Param("email") String email);

    @Query("SELECT new xyz.sadiulhakim.user.pojo.UserDTO(u.id, u.firstname, u.lastname, u.email, u.picture,u.textColor,u.lastSeen) " +
